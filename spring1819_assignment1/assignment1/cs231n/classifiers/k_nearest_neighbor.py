...
class KNearestNeighbor(object):
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            for j in range(k):
                closest_y.append(self.y_train[closest_x[j]])
            ...
            if (k==1): y_pred[i] = closest_y[0]
            else:
                unique, counts = np.unique(closest_y, return_counts=True)
                max_matrix = (counts == np.amax(counts))
                max_unique = np.extract(max_matrix,unique)
                y_pred[i] = max_unique[0]
